...
本プロジェクトは [konbini](https://github.com/kaze-jp/konbini) (SDD + Agent Orchestrator) で開発する。
...
```
.ao/ao.yaml              — konbini 設定 (プリセット, 自律レベル)
.ao/memory/              — レビューパターン学習, プロジェクトコンテキスト
.ao/steering/            — プロジェクトステアリング docs
.claude/agents/          — AO エージェント (orchestrator.md, reviewer.md, implementer.md)
.claude/commands/kiro/   — kiro コマンド定義
.kiro/settings/rules/    — ルールファイル (EARS, 設計原則等)
.kiro/settings/templates/ — テンプレート (spec)
...
<!-- konbini:sdd:start -->
<!-- Generated by konbini v0.2.4 — DO NOT EDIT THIS SECTION MANUALLY -->
## SDD Enforcement Rules (CRITICAL)

This project uses **konbini** (v0.2.4) for Spec-Driven Development (SDD).
