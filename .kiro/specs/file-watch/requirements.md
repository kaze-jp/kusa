...
2. When ファイル削除イベントを受信した場合, フロントエンド shall ユーザーにエラー通知を表示し、プレビューを無効化する
3. When ファイル削除通知が表示された状態でタブが閉じられた場合, kusa shall ファイル一覧画面へ遷移する
...
3. When ユーザーが外部変更の適用を選択した場合, kusa shall ローカル編集を破棄しプレビューを即時更新して「外部変更を適用しました」通知を表示する
4. When ユーザーがローカル変更の維持を選択した場合, kusa shall ローカルバッファを保持し衝突バナーを表示する。次回外部変更検知時に再度衝突警告を表示する
